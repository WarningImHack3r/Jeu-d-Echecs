...
public class Echiquier extends JPanel {
     ...
     */
    boolean cancelBtnIsClickable = false;
    /**
    ...
    public void prediction(Case dep) {
        ...
        if (Echec.joueur == dep.getPiece().getCouleur()) {
            if (dep.getPiece().getType() == TypePiece.PION) { // PION : déplacement d'une ou deux cases vers l'avant,
                                                              // diagonal pour manger
                if (Echec.joueur == CouleurPiece.BLANC) { // if pion blanc else if pion noir
                    ...
                    if (dep.getOrdonnee() >= 0 && dep.getOrdonnee() < 7
                            && monEchiquier[depx + 1][depy + 1].getPiece().getCouleur() == CouleurPiece.NOIR) {
                        monEchiquier[depx + 1][depy + 1].setBorder(Echec.ligneCyan);
                    }
                    if (dep.getOrdonnee() > 0 && dep.getOrdonnee() < 8
                            && monEchiquier[depx + 1][depy - 1].getPiece().getCouleur() == CouleurPiece.NOIR) {
                        monEchiquier[depx + 1][depy - 1].setBorder(Echec.ligneCyan);
                    }
                    if (dep.getAbscisse() == 1 && !monEchiquier[depx + 1][depy].isOccupe()
                            && !monEchiquier[depx + 2][depy].isOccupe()) {
                    ...
                    if (dep.getOrdonnee() >= 0 && dep.getOrdonnee() < 7
                            && monEchiquier[depx - 1][depy + 1].getPiece().getCouleur() == CouleurPiece.BLANC) {
                        monEchiquier[depx - 1][depy + 1].setBorder(Echec.ligneCyan);
                    }
                    if (dep.getOrdonnee() > 0 && dep.getOrdonnee() < 8
                            && monEchiquier[depx - 1][depy - 1].getPiece().getCouleur() == CouleurPiece.BLANC) {
                    ...
                    if (dep.getAbscisse() == 6 && !monEchiquier[depx - 1][depy].isOccupe()
                            && !monEchiquier[depx - 2][depy].isOccupe()) {
                        monEchiquier[depx - 2][depy].setBorder(Echec.ligneVert);
                    }
                }
            ...
            if (dep.getPiece().getType() == TypePiece.TOUR) { // TOUR : déplacement quelconque en ligne droite,
            ...
            if (dep.getPiece().getType() == TypePiece.CAVALIER) { // CAVALIER : en ligne droite, 2 case puis 1 dans une
                                                                  // autre direction
                int[] caseX = { -1, -2, -2, -1, 1, 2, 2, 1 };
                int[] caseY = { -2, -1, 1, 2, 2, 1, -1, -2 };

                for (int i = 0; i < 8; i++) {
                    int abs = depx + caseX[i];
                    int ord = depy + caseY[i];
                    if ((abs >= 0 && abs < 8) && (ord >= 0 && ord < 8)) {
                        if (monEchiquier[abs][ord].isOccupe()
                                && monEchiquier[abs][ord].getPiece().getCouleur() != dep.getPiece().getCouleur()) {
            ...
            if (dep.getPiece().getType() == TypePiece.REINE) { // REINE : déplacement quelconque dans n'importe quelle
                                                               // direction
                predictionsDiagonal(dep);
                predictionsHV(dep);
            }
    ...
    /**
     * Dépendance de prediction(Case dep), elle affiche les prédictions diagonales
     * (Fou et Dame)
     */
    private void predictionsDiagonal(Case dep) {
        ...
        while (i < 8 && j < 8) { // Bas droite
            if (monEchiquier[j][i].isOccupe()
                    && monEchiquier[j][i].getPiece().getCouleur() != dep.getPiece().getCouleur()) {
        ...
        while (k >= 0 && l < 8) { // Bas gauche
            if (monEchiquier[l][k].isOccupe()
                    && monEchiquier[l][k].getPiece().getCouleur() != dep.getPiece().getCouleur()) {
        ...
        while (m >= 0 && n < 8) { // Haut droite
            if (monEchiquier[m][n].isOccupe()
                    && monEchiquier[m][n].getPiece().getCouleur() != dep.getPiece().getCouleur()) {
        ...
        while (o >= 0 && p >= 0) { // Haut gauche
            if (monEchiquier[o][p].isOccupe()
                    && monEchiquier[o][p].getPiece().getCouleur() != dep.getPiece().getCouleur()) {
    ...
    /**
     * Dépendance de prediction(Case dep), elle affiche les prédictions horizontales
     * et verticales (Tour et Dame)
     */
    private void predictionsHV(Case dep) {
        ...
        for (int i = depx + 1; i < 8; i++) { // Bas
            if (monEchiquier[i][depy].isOccupe()
                    && monEchiquier[i][depy].getPiece().getCouleur() != dep.getPiece().getCouleur()) {
        ...
        for (int j = depx - 1; j >= 0; j--) { // Haut
            if (monEchiquier[j][depy].isOccupe()
                    && monEchiquier[j][depy].getPiece().getCouleur() != dep.getPiece().getCouleur()) {
        ...
        for (int k = depy + 1; k < 8; k++) { // Droite
            if (monEchiquier[depx][k].isOccupe()
                    && monEchiquier[depx][k].getPiece().getCouleur() != dep.getPiece().getCouleur()) {
        ...
        for (int l = depy - 1; l >= 0; l--) { // Gauche
            if (monEchiquier[depx][l].isOccupe()
                    && monEchiquier[depx][l].getPiece().getCouleur() != dep.getPiece().getCouleur()) {
    ...
    private Piece popupChangePion(CouleurPiece couleur) {
        ...
        int change = JOptionPane.showOptionDialog(popup,
                "Vous êtes arrivé au bout du camp ennemi. En quel pion voulez-vous vous changer ?", "Jeu d'échecs",
    ...
    private boolean deplacementValide(Case dep, Case arr) {
        return (arr.hasBorders && arr.getBorder() != Echec.ligneRouge)
                && (dep.getPiece().getCouleur() != arr.getPiece().getCouleur());
    }
    ...
    private void jouerCoup(Case dep, Case arr) {
        if (dep.getPiece().getType() == TypePiece.PION && (arr.getAbscisse() == 0 || arr.getAbscisse() == 7)) { // Gestion
                                                                                                                // de
                                                                                                                // l'arrivée
                                                                                                                // d'un
                                                                                                                // pion
    ...
    private Case trouverRoi(CouleurPiece couleur) {
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                if (monEchiquier[i][j].getPiece().getType() == TypePiece.ROI
                        && monEchiquier[i][j].getPiece().getCouleur() != couleur) {
    ...
    public boolean echec(CouleurPiece couleur) {
        ...
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                ...
                if ((caseRoi.getBorder() == Echec.ligneCyan || caseRoi.getBorder() == Echec.ligneMagenta)
                        && couleur == Echec.joueur) { // si le roi ennemi est mangeable...
                    caseRoi.setBorder(Echec.ligneMagenta); // ...l'encadrer

                    // Ajout des coordonnées entre le roi et la pièce qui le met en échec dans le
                    // tableau
                    for (int k = 0; k < 8; k++) {
                        for (int l = 0; l < 8; l++) {
                    ...
                    // true sur les cases entre le roi ennemi et le pion allié (inclus) qui le met
                    // en échec
                    for (int m = 1; m < 8; m++) {
                        if (absRoi + m < 8 && monEchiquier[absRoi + m][ordRoi].getPiece().getType() == typeCaseDep
                                && monEchiquier[absRoi + m][ordRoi].getPiece().getCouleur() != coulRoi) { // Bas
                            for (int n = 1; n <= caseDepAbs; n++) {
                                if (absRoi + n < 8) {
                                    tabSansEchec[absRoi + n][ordRoi] = true;
                                }
                            }
                        } else if (absRoi + m < 8 && ordRoi + m < 8
                                && monEchiquier[absRoi + m][ordRoi + m].getPiece().getType() == typeCaseDep
                                && monEchiquier[absRoi + m][ordRoi + m].getPiece().getCouleur() != coulRoi) { // Bas
                                                                                                              // droite
                            int a = 1, b = 1;
                            ...
                            while (absRoi + a != caseDepAbs + 1 && ordRoi - b != caseDepOrd - 1) {
                                ...
                                && monEchiquier[absRoi - m][ordRoi].getPiece().getType() == typeCaseDep
                                && monEchiquier[absRoi - m][ordRoi].getPiece().getCouleur() != coulRoi) { // Haut
                            for (int n = 1; n <= caseDepAbs; n++) {
                                if (absRoi - n >= 0) {
                                    tabSansEchec[absRoi - n][ordRoi] = true;
                                }
                            }
                        } else if (absRoi - m >= 0 && ordRoi + m < 8
                                && monEchiquier[absRoi - m][ordRoi + m].getPiece().getType() == typeCaseDep
                                && monEchiquier[absRoi - m][ordRoi + m].getPiece().getCouleur() != coulRoi) { // Haut
                                                                                                              // droite
                            int a = 1, b = 1;
                            while (absRoi - a != caseDepAbs && ordRoi + b != caseDepOrd) {
                                tabSansEchec[absRoi - a][ordRoi + b] = true;
                            ...
                            while (absRoi - a != caseDepAbs && ordRoi - b != caseDepOrd) {
                                ...
                                && monEchiquier[absRoi][ordRoi + m].getPiece().getType() == typeCaseDep
                                && monEchiquier[absRoi][ordRoi + m].getPiece().getCouleur() != coulRoi) { // Droite
                            ...
                            for (int n = 1; n <= caseDepAbs; n++) {
                                ...
                                && monEchiquier[absRoi][ordRoi - m].getPiece().getType() == typeCaseDep
                                && monEchiquier[absRoi][ordRoi - m].getPiece().getCouleur() != coulRoi) { // Gauche
                            for (int n = 1; n <= caseDepAbs; n++) {
                                if (ordRoi - n >= 0) {
                                    tabSansEchec[absRoi][ordRoi - n] = true;
                                }
                            ...
                            int[] caseY = { -2, -1, 1, 2, 2, 1, -1, -2 };
                            for (int n = 0; n < 8; n++) {
                                int abs = absRoi + caseX[n];
                                int ord = ordRoi + caseY[n];
                                if ((abs >= 0 && abs < 8) && (ord >= 0 && ord < 8)
                                        && monEchiquier[abs][ord].getPiece().getType() == typeCaseDep
                                        && monEchiquier[abs][ord].getPiece().getCouleur() != coulRoi) {
    ...
    /**
     * Enlève toutes les prédictions qui ne permettent pas aux pièces alliées de
     * sauver leur roi
     */
    public void coupSansEchec() {
        ...
        if (cmptFalse > 0) {
            for (int i = 0; i < 8; i++) {
                for (int j = 0; j < 8; j++) {
                    if ((monEchiquier[i][j].getBorder() == Echec.ligneVert
                            || monEchiquier[i][j].getBorder() == Echec.ligneCyan) && !tabSansEchec[i][j]) { // enlever
                                                                                                            // les
                                                                                                            // prédictions
                                                                                                            // hors
                                                                                                            // du
                        ...
                        monEchiquier[i][j].setBorder(null);
                        monEchiquier[i][j].hasBorders = false;
                    } else if (monEchiquier[i][j].getPiece().getType() == TypePiece.ROI
                            && monEchiquier[i][j].getPiece().getCouleur() == Echec.joueur
                            && monEchiquier[i][j].getBorder() == Echec.ligneRouge) {
                        int[] caseX = { -1, -1, -1, 0, 0, 1, 1, 1 };
                        int[] caseY = { 0, -1, 1, -1, 1, -1, 0, 1 };

                        for (int k = 0; k < 8; k++) {
                            int abs = monEchiquier[i][j].getAbscisse() + caseX[k];
                            int ord = monEchiquier[i][j].getOrdonnee() + caseY[k];
                            if ((abs >= 0 && abs < 8) && (ord >= 0 && ord < 8)) {
                                if (tabSansEchec[abs][ord]) {
    ...
    public boolean echecEtMat(CouleurPiece couleur) {
        ...
        if (echec(Echec.ennemi())) {
            ...
            for (int k = 0; k < 8; k++) {
                for (int l = 0; l < 8; l++) {
                    if (tabSansEchec[k][l] && (monEchiquier[k][l].getBorder() == Echec.ligneVert
                            || monEchiquier[k][l].getBorder() == Echec.ligneCyan)) {
                        casesJouables++;
                    }
                }
            }
            removeNonMagentaBorders();
            return casesJouables == 0;
        }
        return false;
    }

    /**
     * Retourne true si le roi de la couleur _couleur_ ne peut pas bouger sans se
     * mettre en echec
     */
    public boolean pat(CouleurPiece couleur) {
        ...
        for (int m = 0; m < 8; m++) { // Teste si le roi ennemie est entouré de cases vertes, ce qui veut dire que
                                      // nous l'empêchons de bouger
            int abs = roiEnnemi.getAbscisse() + caseX[m];
            int ord = roiEnnemi.getOrdonnee() + caseY[m];
            if ((abs >= 0 && abs < 8) && (ord >= 0 && ord < 8)) {
        ...
        if (autourRoi == dansEchiquier) { // Le roi ennemi ne peut pas bouger
            ...
            for (int m = 0; m < 8; m++) {
                for (int n = 0; n < 8; n++) {
                    if (monEchiquier[m][n].getPiece().getType() != TypePiece.ROI
                            && monEchiquier[m][n].getPiece().getCouleur() == Echec.ennemi()) { // teste si les pions
                                                                                               // ennemis peuvent bouger
                        nbPiece++;
                        prediction(monEchiquier[m][n]);
                        for (int o = 0; o < 8; o++) {
                            for (int p = 0; p < 8; p++) {
                                if (monEchiquier[o][p].getBorder() == Echec.ligneVert
                                        || monEchiquier[o][p].getBorder() == Echec.ligneCyan) {
                                    nbBorder++;
                                }
                            }
                        }
    ...
    public boolean finJeu(CouleurPiece couleur) {
        ...
        if (isFin == 1) {
            ...
            int finJeu = JOptionPane.showOptionDialog(finJeuWD, stringFin + " Que voulez-vous faire ?", "Jeu d'échecs",
                    JOptionPane.DEFAULT_OPTION, JOptionPane.PLAIN_MESSAGE, null, options, options[0]);
            if (finJeu == 0) {
                reinitialiser();
            } else if (finJeu == 1) {
    ...
    public void reinitialiser() {
        ...
        int reset = JOptionPane.showOptionDialog(resetWD, "Voulez-vous vraiment recommencer la partie ?",
                "Jeu d'échecs", JOptionPane.DEFAULT_OPTION, JOptionPane.PLAIN_MESSAGE, null, options, options[0]);
        if (reset == 0) {
            initialiser();
            for (int i = 0; i < 8; i++) {
                for (int j = 0; j < 8; j++) {
    ...
    public void annulerCoup(Case dep, Case arr) {
        if (dep.getPiece().getCouleur() != Echec.joueur || arr.getPiece().getCouleur() != Echec.joueur) {
            ...
            if (tempMange != null && dep.getPiece().getCouleur() != arr.getPiece().getCouleur()) {
    ...
    public void leaveGame() {
        ...
        int leave = JOptionPane.showOptionDialog(leaveWD, "Voulez-vous vraiment quitter le jeu ?", "Jeu d'échecs",
                JOptionPane.DEFAULT_OPTION, JOptionPane.PLAIN_MESSAGE, null, options, options[0]);
        if (leave == 0) {
            System.exit(0);
        } else {
